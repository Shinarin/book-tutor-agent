...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Save embedded images as files in this directory and reference them "
            "from the Markdown by relative path. If omitted and -o is given, "
            "defaults to <output_basename>_images next to the output file. In "
            "stdout mode with no explicit directory, images are not saved. "
            "Takes precedence over --keep-data-uris when both are set."
        ),
    )
    ...
    md_output_dir = None
    if args.output:
        md_output_dir = os.path.dirname(os.path.abspath(args.output)) or "."
        if save_images_dir is None:
            stem = os.path.splitext(os.path.basename(args.output))[0] or "output"
            save_images_dir = os.path.join(md_output_dir, f"{stem}_images")
